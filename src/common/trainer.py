...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    Additional details can be documented in the project README. `model` is the instantiated object of a Model Class.
    ...
    def _save_best_embeddings(self):
        save_name = self.config['log_file_name'] or '{}_{}_best'.format(self.config['model'], self.config['dataset'])
        save_dir = os.path.join('saved_models', self.config['model'], self.config['dataset'], save_name)
        os.makedirs(save_dir, exist_ok=True)

        was_training = self.model.training
        self.model.eval()
        with torch.no_grad():
            if hasattr(self.model, 'export_embeddings'):
                embeddings = self.model.export_embeddings()
            else:
                outputs = self.model.forward()
                user_emb = outputs[0]
                item_emb = outputs[1]
                # The shared rerank script expects content/mm views. Models that
                # expose one final representation duplicate it here.
                embeddings = {
                    'all_user': user_emb,
                    'all_item': item_emb,
                    'content_user': user_emb,
                    'content_item': item_emb,
                    'mm_user': user_emb,
                    'mm_item': item_emb,
                }
        ...
        for name, tensor in embeddings.items():
            ...
            np.save(os.path.join(save_dir, '{}_emb.npy'.format(name)), arr)
        torch.save(self.model.state_dict(), os.path.join(save_dir, 'model.pth'))
        if hasattr(self.model, 'export_diagnostics'):
            diagnostics = self.model.export_diagnostics()
            with open(os.path.join(save_dir, 'diagnostics.json'), 'w', encoding='utf-8') as fout:
                json.dump(diagnostics, fout, indent=2, sort_keys=True)

        if was_training:
            self.model.train()
        self.logger.info('Saved best embeddings to {}'.format(save_dir))
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if update_flag:
                    ...
                    if saved:
                        self._save_best_embeddings()
                    if hasattr(self.model, 'diagnostic_info') and verbose:
                        self.logger.info(self.model.diagnostic_info())
